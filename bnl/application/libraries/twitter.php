...
class Twitter {
    ...
    public function oauth($consumer_key, $consumer_secret, $access_token = NULL, $access_token_secret = NULL) {
        ...
        if ( $access_token === NULL && $access_token_secret === NULL && !isset($_REQUEST['oauth_token']) ) {
            $url = $this->oauth->getAuthorizationUrl();
            
            header('Location: '.$url);
        }
        elseif ( $access_token === NULL && $access_token_secret === NULL && isset($_REQUEST['oauth_token']) ) {
            $access_token = $_REQUEST['oauth_token'];
...
class EpiOAuth {
    ...
    public function getAuthorizeUrlFs($token = null, $params = null) {
        $token = $token ? $token : $this->getRequestToken($params);
