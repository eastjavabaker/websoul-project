...
<?php } ?>
<h3><?php if(preg_match("/websoul\/member/",$_SERVER['REQUEST_URI'])) { echo "All Member List"; } else { echo "Competition Registrant "; }?></h3>
<?php if(preg_match("/websoul\/quiz/",$_SERVER['REQUEST_URI'])) { ?>
<strong>Date Period :</strong><br><br>
<form action="<?php echo site_url("websoul/member/period");?>" method="post" >
...
<div class="pagination" style="padding-bottom: 10px;"><?php echo $paging." ( $total record(s) )"; ?></div>
	...
	<th width="120">Mesin Cuci</th>
	<?php if(preg_match("/websoul\/quiz/",$_SERVER['REQUEST_URI'])) { ?><th width="200">Cover</th><?php }?>
	...
	<td valign="top"><?php echo $v->name."<br>( FBUID : ".$v->fb_uid." )";?></td>
        <td valign="top"><?php echo $v->email;?></td>
	<td valign="top"><?php if($v->picture2){?><img border="0" src="<?php echo base_url('data/mesincuci').'/'.$v->picture2;?>" /><?php }else{ ?>-<?php } ?></td>
	<?php if(preg_match("/websoul\/quiz/",$_SERVER['REQUEST_URI'])) { ?><td valign="top"><a href="<?php echo base_url('data/cover').'/'.$v->picture;?>" target="_blank" ><img border="0" src="<?php echo base_url('data/cover').'/'.$v->thumbnail;?>" /></a></td><?php } ?>
        <?php if(preg_match("/websoul\/quiz/",$_SERVER['REQUEST_URI'])) { ?>
